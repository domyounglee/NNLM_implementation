...
void InitNet() {
	...
	if (negative>0) {
		...
		a = posix_memalign((void **)&syn1neg, 128, (long long)layer2_size *(layer1_size+1)* sizeof(real));
		...
		//1layer-2layer  matrix with bias 
		a = posix_memalign((void **)&gradsq_syn1neg, 128, (long long)layer2_size *(layer1_size+1)* sizeof(real));
		if (gradsq_syn1neg == NULL) {printf("Memory allocation failed\n"); exit(1);}
		for (a = 0; a < layer2_size; a++) for (b = 0; b <= layer1_size; b++){
		next_random = next_random * (unsigned long long)25214903917 + 11;
		 gradsq_syn1neg[a * layer1_size + b] = 1.0;
		}


		//2layer-output matrix  with bias 
		a = posix_memalign((void **)&syn2neg, 128, (long long)vocab_size * (layer2_size+1) * sizeof(real));
		...
		//2layer-output matrix  with bias 
		a = posix_memalign((void **)&gradsq_syn2neg, 128, (long long)vocab_size * (layer2_size+1) * sizeof(real));
		if (gradsq_syn2neg == NULL) {printf("Memory allocation failed\n"); exit(1);}
		for (a = 0; a < vocab_size; a++) for (b = 0; b <= layer2_size; b++)
		 gradsq_syn2neg[a * layer2_size + b] = 1.0;
	...
	for (a = 0; a < vocab_size; a++) for (b = 0; b < feature_size; b++) {
		next_random = next_random * (unsigned long long)25214903917 + 11;
		gradsq_syn0[a * feature_size + b] = 1.0;
	}
	...
	while (1){
		...
		for (c = 0; c < layer2_size; c++) {
			for (b = 0; b < layer1_size; b++) {
				...
				neu2[c]+= neu1[b] * syn1neg[c + b*layer2_size]; //h2=w'*h1 (  c is  row,b is column )
			}
			
		}
		for (c = 0; c < layer2_size; c++) {
			if (neu2[c]> 3) neu2[c]=1.0;
			else if (neu2[c]< -3) neu2[c]=-1.0;
		...
		if (cw) {	
			...
			if (negative > 0) for (d = 0; d < negative + 1; d++) {
				...
				for (c = 0; c < layer2_size; c++){
					for (b = 0; b < layer1_size; b++) {
						//accumulate the error of layer1 to update syn0 later
						neu1e[b] += g * syn2neg[c + l2]*(1-neu2[c]*neu2[c])*syn1neg[c+b*layer2_size];
					}
					//accumulate the error of layer2 to update syn1 later
					neu2e[c] += g * syn2neg[c + l2];
				} 


				// update  bias syn2neg 
				 syn2neg[layer2_size+ l2]+=alpha*g/sqrt(gradsq_syn2neg[layer2_size+ l2]+1e-6);
				 gradsq_syn2neg[layer2_size+ l2]+=pow(g,2);


				//update  syn2neg
				for(c=0;c<layer2_size;c++){
					syn2neg[c + l2] += alpha * g * neu2[c]    /sqrt(gradsq_syn2neg[c + l2]+1e-6);
					gradsq_syn2neg[c + l2]+=pow(g * neu2[c],2);
				} 
			...
			for(c=0 ; c<layer2_size ; c++){
				for(b=0 ; b<layer1_size ; b++){
					syn1neg[c+b*layer2_size]+= alpha*  neu2e[c]*(1-neu2[c]*neu2[c])*neu1[b]    /sqrt(gradsq_syn1neg[c+b*layer2_size]+1e-6);
					gradsq_syn1neg[c+b*layer2_size]+=pow(neu2e[c]*(1-neu2[c]*neu2[c])*neu1[b],2);
				}
				// update  bias syn1neg 
				syn1neg[c+layer1_size*layer2_size]+= alpha*    neu2e[c]*(1-neu2[c]*neu2[c])    /sqrt(gradsq_syn1neg[c+layer1_size*layer2_size]+1e-6);
				gradsq_syn1neg[c+layer1_size*layer2_size]+=pow(neu2e[c]*(1-neu2[c]*neu2[c]),2);
			}
			...
			for (a = 0; a < window * 2 + 1 ; a++) if (a != window) {
				...
				for (c = 0; c < feature_size; c++){
					//printf("%lf ",neu1e[c+b*feature_size] );
					syn0[c + last_word * feature_size] += alpha*   neu1e[c+b*feature_size]    /sqrt(gradsq_syn0[c + last_word * feature_size]+1e-6);
					gradsq_syn0[c + last_word * feature_size]+=pow(neu1e[c+b*feature_size],2);
				} 
				b++;
			}
